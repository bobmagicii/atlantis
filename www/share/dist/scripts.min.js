...
CodeMirror.requireMode=function(mode,cont,options){if(typeof mode!="string")mode=mode.name;if(CodeMirror.modes.hasOwnProperty(mode))return ensureDeps(mode,cont,options);if(loading.hasOwnProperty(mode))return loading[mode].push(cont);var file=options&&options.path?options.path(mode):CodeMirror.modeURL.replace(/%N/g,mode);if(options&&options.loadMode){options.loadMode(file,function(){ensureDeps(mode,cont,options)})}else if(env=="plain"){var script=document.createElement("script");script.src=file;var others=document.getElementsByTagName("script")[0];var list=loading[mode]=[cont];CodeMirror.on(script,"load",function(){ensureDeps(mode,function(){for(var i=0;i<list.length;++i)list[i]();},options)});others.parentNode.insertBefore(script,others)}else if(env=="cjs"){require(file);cont()}else if(env=="amd"){requirejs([file],cont)}};CodeMirror.autoLoadMode=function(instance,mode,options){if(!CodeMirror.modes.hasOwnProperty(mode))
CodeMirror.requireMode(mode,function(){instance.setOption("mode",instance.getOption("mode"))},options)}});(function(mod){if(typeof exports=="object"&&typeof module=="object")
...
if(info.alias[j].toLowerCase()==name)return info}}});"use strict";var Atlantis={};Atlantis.CopyToClipboard=function(What){let that=this;let Textbox;let OgText;jQuery(that).append(Textbox=jQuery('<textarea />').css({'width':'0px','height':'0px','border':'0px;'}).val(What));Textbox.select();document.execCommand('copy');Textbox.remove();OgText=jQuery(that).text();jQuery(that).text('Copied!');setTimeout(function(){jQuery(that).text(OgText)},1000);return!1};Atlantis.CopyElementToClipboard=function(){let ElementID=jQuery(this).attr('data-copy-element');let Value=jQuery.trim(jQuery(ElementID).text());(Atlantis.CopyToClipboard).call(this,Value);return!1};Atlantis.CopyValueToClipboard=function(){e.preventDefault();let Value=jQuery.trim(jQuery(this).attr('data-copy-value'));(Atlantis.CopyToClipboard).call(this,Value);return!1};jQuery(document).ready(function(){CodeMirror.modeURL="/share/dist/src/codemirror-modes/%N/%N.js";jQuery('.CopyElementToClipboard').on('click',Atlantis.CopyElementToClipboard);jQuery('.CopyValueToClipboard').on('click',Atlantis.CopyValueToClipboard);jQuery('.HasTooltip').each(function(){let that=jQuery(this);let Opts={};that.tooltip(Opts);return});jQuery('.btn-toggle').on('click',function(){if(jQuery(this).hasClass('disabled'))
return;jQuery(this).toggleClass('active');return});jQuery('.CodeViewer').each(function(){let Element=jQuery(this);if(Element.parents('.EditorContainer').length>0)
return;let Editor=null;let Container=null;let Lang=Element.attr('data-lang')??'txt';let LangData=CodeMirror.findModeByExtension(Lang);let Theme=Element.attr('data-theme');Element.after(Container=jQuery('<div />').addClass('CodeViewer'));Element.hide();Editor=CodeMirror(Container.get(0),{'value':jQuery.trim(Element.text()),'lineNumbers':!0,'indentWithTabs':!0,'readOnly':!0,'indentUnit':4,'tabSize':4});if(LangData&&LangData.mode)
CodeMirror.autoLoadMode(Editor,LangData.mode);if(LangData&&LangData.mime)
Editor.setOption('mode',LangData.mime);if(Theme)
Editor.setOption('theme',Theme);return});return});"use strict";if(typeof Atlantis.Action==='undefined')
Atlantis.Action={};Atlantis.Action.StatusHandler=function(Opt){let that=this;let Config={'Element':null,'ElementCommit':'.ActionCommit','ElementStatusAll':'.ActionStatus','ElementStatusThink':'.ActionStatusThink','ElementStatusGood':'.ActionStatusGood','ElementStatusBad':'.ActionStatusBad','CommitAction':'click','OnCommit':function(Handler){Handler.SetStatus('good');return},'OnKeyPress':function(Handler){Handler.SetStatus(null);return}};this.SetStatus=function(Mode){(this.Status.All).addClass('font-size-zero');switch(Mode){case 'think':(this.Status.Think).removeClass('font-size-zero');break;case 'good':(this.Status.Good).removeClass('font-size-zero');break;case 'bad':(this.Status.Bad).removeClass('font-size-zero');break};return this};this.Notify=function(ToastOpt){Atlantis.Toaster(ToastOpt);return this};NUI.Util.MergeProperties(Opt,Config);this.Element=Config.Element;this.Commit=Config.ElementCommit;this.Status={'All':Config.ElementStatusAll,'Think':Config.ElementStatusThink,'Good':Config.ElementStatusGood,'Bad':Config.ElementStatusBad};if(typeof this.Element==='string')
this.Element=jQuery(this.Element);if(typeof this.Commit==='string')
this.Commit=this.Element.find(this.Commit);for(Prop in this.Status)
if(typeof this.Status[Prop]==='string')
this.Status[Prop]=this.Element.find(this.Status[Prop]);if(this.Commit.is('a, btn, input[type=button], input[type=submit]'))
Config.CommitAction='click';else if(this.Commit.is('input[type=checkbox], input[type=radio], select'))
Config.CommitAction='change';(this.Element.find('input')).on('keypress',function(){if(typeof Config.OnKeyPress==='function')(Config.OnKeyPress)(that);return});(this.Commit).on(Config.CommitAction,function(){that.SetStatus('think');if(typeof Config.OnCommit==='function')
setTimeout(function(){(Config.OnCommit)(that);return},300);return!1});return this};Atlantis.BlogPost={'Delete':function(PostID){(new Promise(function(Next,Fail){Atlantis.Request({'Method':'DELETE','URL':'/api/v1/post/entity','Data':{'ID':PostID},'OnSuccess':function(Result){Next(Result);return}})})).then(function(Result){if(typeof Result.Location==='string'){location.href=Result.Location;return}
return});return}};jQuery(document).ready(function(){jQuery('.AtlantisActionPostDelete').on('click',function(){let PostID=parseInt(jQuery(this).attr('data-post-id'));if(PostID>0)
if(confirm('Really delete this post? This cannot be undone.'))
Atlantis.BlogPost.Delete(PostID);return!1});return});"use strict";Atlantis.Request=function(Opt){var Config={Method:'Get',URL:'/no/url/lol',Data:null,IsFormData:!1,OnSuccess:null,OnError:null};NUI.Util.MergeProperties(Opt,Config);let Request={type:Config.Method.toUpperCase(),url:Config.URL,data:Config.Data,dataType:'json'};if(Config.IsFormData){Request.processData=!1;Request.contentType=!1;Request.mimeType='multipart/form-data'}
jQuery.ajax(Request).done(function(Result){if(Result.Error!=0){if(typeof Config.OnError=='function')
Config.OnError(Result);else alert(Result.Message);return}
if(typeof Config.OnSuccess=='function')
Config.OnSuccess(Result);return});return};"use strict";Atlantis.Toaster=function(Opt){let Config={'Title':'Notification','Content':'Consider yourself notified.','Icon':'fa-cog','ContentClass':!1,'AutoHide':!0,'Delay':3000};NUI.Util.MergeProperties(Opt,Config);let Header=null;let Content=null;let Toast=jQuery('<div />').addClass('toast m-3').attr('role','alert');if(Config.Title!==!1){Toast.append(Header=jQuery('<div />').addClass('toast-header').append(jQuery('<i />').addClass('rounded mr-2 fa fas').addClass(Config.Icon)).append(jQuery('<strong />').addClass('mr-auto').text(Config.Title)).append(jQuery('<button />').addClass('ml-2 mb-1 close').attr('type','button').attr('data-dismiss','toast').html('<span>&times;</span>')))}
if(Config.Content!==!1){Toast.append(Content=jQuery('<div />').addClass('toast-body').append(Config.Content));if(Config.ContentClass)
Content.addClass(Config.ContentClass)}
jQuery('#Toaster').append(Toast);Toast.toast({'autohide':Config.AutoHide,'delay':Config.Delay}).toast('show');return};'use strict';Atlantis.Upload={FileTypeAny:0,FileTypeImage:1};'use strict';Atlantis.BlogTag={};Atlantis.BlogTag.DialogRename=function(TagID,Opt){let that=this;let Overlay;let Dialog;let InputText;let Prepare;let Construct;let Commit;let Cancel;let Config={'OnSuccess':function(Diag,Result){location.reload(!0);return}};NUI.Util.MergeProperties(Opt,Config);Prepare=function(){Atlantis.Request({'Method':'TAGGET','URL':'/api/v1/blog/entity','Data':{'TagID':TagID},'OnSuccess':function(Result){Construct(Result);return}});return};Construct=function(Result){Overlay=new NUI.Overlay({'Content':(Dialog=new NUI.Dialog({'Title':'Rename Tag','Content':(jQuery('<div />').addClass('row').append(jQuery('<div />').addClass('col-12').append(InputText=jQuery('<input />').attr('type','text').addClass('form-control').val(Result.Payload.Title)))),'Buttons':[new NUI.Button({'Label':'Save','Class':'NUI-Dialog-Accept btn btn-primary'}),new NUI.Button({'Label':'Cancel','Class':'NUI-Dialog-Cancel btn btn-dark'})],'OnAccept':function(Result){Commit();return},'OnCancel':function(Result){Cancel();return}}))});return};Commit=function(){let TagTitle=InputText.val();Atlantis.Request({'Method':'TAGPATCH','URL':'/api/v1/blog/entity','Data':{'TagID':TagID,'TagTitle':TagTitle},'OnSuccess':function(Result){if(typeof Config.OnSuccess==='function')(Config.OnSuccess)(that,Result);return}});return};Cancel=function(){Overlay.Destroy();return};this.Close=Cancel;Prepare();return this};Atlantis.BlogTag.DialogDelete=function(TagID,Opt){let that=this;let Overlay;let Dialog;let InputText;let Prepare;let Construct;let Commit;let Cancel;let Config={'OnSuccess':function(Diag,Result){location.reload(!0);return}};NUI.Util.MergeProperties(Opt,Config);Prepare=function(){Atlantis.Request({'Method':'TAGGET','URL':'/api/v1/blog/entity','Data':{'TagID':TagID},'OnSuccess':function(Result){Construct(Result);return}});return};Construct=function(Result){Overlay=new NUI.Overlay({'Content':(Dialog=new NUI.Dialog({'Title':'Delete Tag','Content':(jQuery('<div />').addClass('row').append(jQuery('<div />').addClass('col-12').html('Really delete <b>'+Result.Payload.Title+'</b>? This cannot be undone.'))),'Buttons':[new NUI.Button({'Label':'Delete','Class':'NUI-Dialog-Accept btn btn-danger'}),new NUI.Button({'Label':'Cancel','Class':'NUI-Dialog-Cancel btn btn-dark'})],'OnAccept':function(Result){Commit();return},'OnCancel':function(Result){Cancel();return}}))});return};Commit=function(){Atlantis.Request({'Method':'TAGDELETE','URL':'/api/v1/blog/entity','Data':{'TagID':TagID},'OnSuccess':function(Result){if(typeof Config.OnSuccess==='function')(Config.OnSuccess)(that,Result);return}});return};Cancel=function(){Overlay.Destroy();return};this.Close=Cancel;Prepare();return this};'use strict';if(typeof Atlantis.Editor==='undefined')
Atlantis.Editor={};Atlantis.Editor.EditorJS=function(Opt){let that=this;this.Container=null;this.Element=null;this.Instance=null;let Config={'SelectEditor':null};NUI.Util.MergeProperties(Opt,Config);if(typeof Config.SelectEditor!=='string'){console.log('No valid SelectEditor selector.');return}
this.GetContent=async function(){let Result=null;try{Result=await that.Instance.save()}catch(ErrMsg){console.log(`EditorJS Error: ${ErrMsg}`)}
return Result}(async function(){that.Container=jQuery(Config.SelectEditor);that.Element=that.Container.find('.Editor');that.Instance=new EditorJS({'holder':that.Element[0],'placeholder':'Hello World...'});try{await that.Instance.isReady;that.Container.find('.Throbber').hide();that.Element.addClass('Ready')}catch(ErrMsg){console.log(`Editor.js Error: ${ErrMsg}`)}
return})();return this}
'use strict';Atlantis.Upload.Button=function(Opt){let that=this;let Config={'SelectorButton':null,'SelectorInput':null,'SelectorUpload':null,'FileType':Atlantis.Upload.FileTypeAny,'AutoUpload':!0,'Method':'POST','URL':null,'Data':null,'OnItemComplete':function(that,Status,Result,File){if(Result.Error!==0)
alert(Result.Message);return},'OnQueueComplete':function(that){location.reload(!0);return}};let OnButtonClick;let OnSelectFiles;let OnFileTypeError;let OnQueueNext;let UploadFile;let Queue=new Array;NUI.Util.MergeProperties(Opt,Config);this.Button=jQuery(Config.SelectorButton);this.Input=jQuery(Config.SelectorInput);this.Reset=function(){(that.Button.find('.StatusThinking')).addClass('font-size-zero');(that.Button.find('.UploadButtonBar')).css('width','0%');return};OnButtonClick=function(){that.Input.trigger('click');return!1};OnSelectFiles=function(){jQuery(this.files).each(function(){console.log('Upload Icon: '+this.name+' '+this.type);if(Config.FileType===Atlantis.Upload.FileTypeImage){if(!this.type.match(/^image\//)){OnFileTypeError(this);return!1}}
Queue.push(this);return});if(Config.AutoUpload)
OnQueueNext();return};OnFileTypeError=function(File){let Overlay;let Dialog;Overlay=new NUI.Overlay({'Content':(Dialog=new NUI.Dialog({'Title':'Invalid File Type','Buttons':[new NUI.Button({'Label':'OK','Class':'NUI-Dialog-Cancel btn btn-dark'})],'Content':File.name,'OnCancel':function(){Overlay.Destroy();return}}))});return};OnQueueNext=function(){let Next=Queue.shift();if(typeof Next==='object'){UploadFile(Next);return}
if(typeof Config.OnQueueComplete==='function')(Config.OnQueueComplete)(that);return};UploadFile=function(File){let Uploader=new FormData;let Xfer=new XMLHttpRequest();let Prop=null;(that.Button.find('.StatusThinking')).removeClass('font-size-zero');(Xfer.upload).addEventListener('progress',function(Ev){let Per=0;if(Ev.lengthComputable){Per=Math.round((Ev.loaded*100)/Ev.total);(that.Button.find('.UploadButtonBar')).css('width',(Per+'%'))}
return},!1);(Xfer).addEventListener('load',function(Ev){if(typeof Config.OnItemComplete==='function')(Config.OnItemComplete)(that,Xfer.status,Xfer.response,File);OnQueueNext();return});if(typeof Config.Data==='object')
for(Prop in Config.Data)
Uploader.append(Prop,Config.Data[Prop]);Uploader.append('Filedata',File);Xfer.responseType='json';Xfer.open(Config.Method,Config.URL);Xfer.send(Uploader);return};(this.Button).css('position','relative').append(jQuery('<i />').addClass('Status StatusThinking fas fa-cog fa-spin font-size-anim font-size-normal font-size-zero ml-2')).append(jQuery('<div />').addClass('UploadButtonBar').css({'position':'absolute','top':'0px','left':'0px','width':'0%','height':'100%','background-color':'#cccc'})).on('click',OnButtonClick);(this.Input).on('change',OnSelectFiles);return this}